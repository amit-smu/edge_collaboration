...
if __name__ == "__main__":
    ...
    img_output_dir = "dataset/Wildtrack_dataset_v1/PNGImages/"
    ...
    with open(training_files) as train_files:
        ...
        while len(filename) > 2:
            ...
            prior = np.full(shape=(img_height, img_width, 1), fill_value=114, dtype=np.uint8)
            # geenrate random overlap region
            ...
            temp = cv2.resize(region, dsize=(region_w_tr, region_h_tr))
            region = cv2.resize(temp, dsize=(region_w, region_h))
            ...
            # print(image.shape)
            # print(prior.shape)
            image = np.concatenate((image, prior), axis=2)
            # channel_initials = "RGBP"
            # for channel_index in range(4):
            #     channel = np.zeros(shape=image.shape, dtype=np.uint8)
            #     channel[:, :, channel_index] = image[:, :, channel_index]
            #     cv2.imshow('{}'.format(channel_initials[channel_index]), channel)
            cv2.imwrite("{}/{}".format(img_output_dir, filename), image)
