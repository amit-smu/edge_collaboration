...
class Evaluator:
    ...
    def __init__(self,
        ...
        # print("testing for resolution: {}".format(self.target_resolution))
        ##################################################
    ...
    # def change_resolution(self, batch_x):
    #     if self.target_resolution == (300, 300):
    #         return batch_x
    #
    #     batch_x_1 = []
    #     # print(batch_x.shape)
    #     for batch_item in batch_x:
    #         img_resized = cv2.resize(src=batch_item, dsize=self.target_resolution)
    #         img_resized = cv2.resize(src=img_resized, dsize=(300, 300))
    #         batch_x_1.append(img_resized)
    #
    #     batch_x_1 = np.array(batch_x_1)
    #     return batch_x_1

    def dump_gen_data(self, batch_x_data, batch_x_aux_data):
        temp_dir = "./temp"
        for bx_data, bax_data in zip(batch_x_data, batch_x_aux_data):
            img_file_name = "{}/{}.jpg".format(temp_dir, self.temp_index)
            mask_file_name = "{}/{}_mask.jpg".format(temp_dir, self.temp_index)
    ...
    def predict_on_dataset(self,
        ...
        for j in tr:
            # Generate batch.
            [batch_X,
             batch_X_aux], batch_image_ids, batch_eval_neutral, batch_inverse_transforms, batch_orig_labels = next(
                generator)

            #########################################################################################
            # dump generator data for verification
            self.dump_gen_data(batch_x_data=batch_X, batch_x_aux_data=batch_X_aux)
            ########################### add multi resolution support here #############################

            # batch_X = self.change_resolution(batch_X)

            ############################################################################################
            # Predict.
            y_pred = self.model.predict([batch_X, batch_X_aux])
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            cumulative_true_pos = np.cumsum(true_pos)  # Cumulative sums of the true positives
            cumulative_false_pos = np.cumsum(false_pos)  # Cumulative sums of the false positives
            print("cumulative_true_positives :{}".format(cumulative_true_positives))
            print("cumulative_false_positives : {}".format(cumulative_false_positives))
            cumulative_true_positives.append(cumulative_true_pos)
            cumulative_false_positives.append(cumulative_false_pos)
