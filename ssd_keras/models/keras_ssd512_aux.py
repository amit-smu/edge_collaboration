...
def ssd_512_aux(image_size,
    ...
    aux_tensor = Input(shape=(img_height, img_width, 1))  # returns a tensor
    print("shape of x : {}".format(x.shape))
    print("shape of aux_tensor : {}".format(aux_tensor.shape))

    # The following identity layer is only needed so that the subsequent lambda layers can be optional.
    x1 = Lambda(identity_layer, output_shape=(img_height, img_width, img_channels), name='identity_layer')(x)

    aux_input = Lambda(identity_layer, output_shape=(img_height, img_width, 1), name='identity_layer_aux')(aux_tensor)
    ...
    #############################################################################
    # concatenate two tensors. Add a Concatenation layer
    #############################################################################
    concat_input = Concatenate(axis=3, name="aux_concat_layer")([x1, aux_input])
    ...
    #                  kernel_regularizer=l2(l2_reg), name='conv1_1_1')(x1)
    conv1_1 = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_initializer='he_normal',
                     kernel_regularizer=l2(l2_reg), name='conv1_1_1')(concat_input)
    ...
    if mode == 'training':
        ...
        model = Model(inputs=[x, aux_tensor], outputs=predictions)
    elif mode == 'inference':
        ...
        model = Model(inputs=[x, aux_tensor], outputs=decoded_predictions)
