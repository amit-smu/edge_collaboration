...
class DataGenerator:
    ...
    def translate_coordinates(self, coordinates, org_width, org_height):
        xmin1 = int((coordinates[0] / org_width) * self.test_resolution[0])
        ymin1 = int((coordinates[1] / org_height) * self.test_resolution[1])
        xmax1 = int((coordinates[2] / org_width) * self.test_resolution[0])
        ymax1 = int((coordinates[3] / org_height) * self.test_resolution[1])

        return xmin1, ymin1, xmax1, ymax1

    def mark_box_coords_PETS(self, annot_path, aux_channel, reg_model):
        # load object coordinates from xml
        root = ElementTree.parse(annot_path).getroot()
        ...
        annot_height = int(size[1].text)
        # print(annot_path)
        # print("annot_width : {}, annot_height : {}".format(annot_width, annot_height))
        ...
        for obj in objects:
            if obj[0].text == "person":
                ...
                if reg_model is None:
                    aux_channel[ymin:ymax, xmin:xmax] = 255
                    # cv2.rectangle(aux_channel, (xmin, ymin), (xmax, ymax), (0, 255, 0), 2)
                else:
                    # transform points to destination view point
                    # find bottom mid point
                    org_box_height = ymax - ymin
                    org_box_width = xmax - xmin
                    bot_mid_x = int(xmin + org_box_width / 2)
                    bot_ymax = ymax
                    box_pred = reg_model.predict([[bot_mid_x, bot_ymax]])
                    box_pred = box_pred[0]
                    bot_mid_x_t, bot_ymax_t = box_pred
                    # convert to bounding box
                    org_box_width = 2 * org_box_width  # adjust the width for marking bigger area
                    dst_box_xmin = int(bot_mid_x_t - org_box_width / 2)
                    dst_box_xmax = int(bot_mid_x_t + org_box_width / 2)
                    dst_box_ymin = int(bot_ymax_t - org_box_height)
                    dst_box_ymax = int(bot_ymax_t)

                    # print(dst_box_xmin, dst_box_ymin, dst_box_xmax, dst_box_ymax)

                    aux_channel[dst_box_ymin:dst_box_ymax, dst_box_xmin:dst_box_xmax] = 255
                    # aux_channel[ymin:ymax, xmin:xmax] = 255

        return aux_channel

    def get_aux_channel_PETS(self, img_height, img_width, batch_file_names):
        """
        aux channel generator for PETS dataset
        :param img_height:
        :param img_width:
        ...
        """
        # reference_cam = [7]
        # collaborating_cam = [8, 6, 5]
        batch_x_auxillary = []
        reference_cam = 7
        collaborating_cams = [8, 6, 5]

        # create regression model directory
        t_img_path = batch_file_names[0]  # sample : ../../dataset/PETS/JPEGImages/78_0001.jpg
        d_dir = t_img_path.split("JPEGImages")[0]
        reg_models_dir = "{}/{}".format(d_dir, "regression_models")
        # load regression models here
        reg_models = []  # regression based models for each camera combination
        if self.N >= 1:
            with open("{}/regression_87_bot_pts".format(reg_models_dir), 'rb') as input_f:
                r_model = pickle.load(input_f)
                reg_models.append(r_model)

        if self.N >= 2:
            with open("{}/regression_67_bot_pts".format(reg_models_dir), 'rb') as input_f:
                r_model = pickle.load(input_f)
                reg_models.append(r_model)

        if self.N >= 3:
            with open("{}/regression_57_bot_pts".format(reg_models_dir), 'rb') as input_f:
                r_model = pickle.load(input_f)
                reg_models.append(r_model)
        ...
        for i in range(len(batch_file_names)):
            ...
            # aux_channel = None
            img_path = batch_file_names[i]  # sample : /dataset/PETS/JPEGImages/78_0001.jpg

            # generate annotation path for auxillary input
            frame_num = img_path[-8:-4]
            t_split = img_path.split("JPEGImages/")
            # frame_num = t_split[1].split("_")[1][:-4]

            # iterate over all collaborating cams
            for c in range(0, self.N):
                annot_name = "frame_{}{}_{}.xml".format(collaborating_cams[c], reference_cam, frame_num)
                # annot_name = "frame_{}{}_{}.xml".format(7, 8, frame_num)
                annot_path = "{}Annotations/{}".format(t_split[0], annot_name)

                ############################# temp code #################
                # temp_img_path = annot_path.replace("Annotations", "JPEGImages")
                # temp_img_path = "{}.jpg".format(temp_img_path[:-4])
                #
                # temp_img = cv2.imread(temp_img_path, cv2.IMREAD_GRAYSCALE)
                # temp_img = cv2.resize(temp_img, (300, 300))
                # temp_img = np.reshape(temp_img, (300, 300, 1))
                # aux_channel = temp_img
                ########################## end temp code #######################
                # aux_channel = self.mark_box_coords_PETS(annot_path=annot_path, aux_channel=aux_channel,
                #                                         reg_model=reg_models[c])
                aux_channel = self.mark_box_coords_PETS(annot_path=annot_path, aux_channel=aux_channel,
                                                        reg_model=None)
            # aux_channel = np.full((img_height, img_width, 1), 255, dtype=np.uint8)
        ...
        # annot_name = "{}{}{}".format(t_split[1][1], t_split[1][0], t_split[1][2:])  # 87_0001.jpg
        # annot_name = annot_name[:-4] + ".xml"
        # annot_name = "{}JPEGImages/{}".format(t_split[0], annot_name)
    ...
    def get_aux_channels(self, img_height, img_width, batch_file_names):
        ...
        """
        batch_x_auxillary = []
        # batch_size = len(batch_file_names)

        for i in range(len(batch_file_names)):
            aux_channel = np.full((img_height, img_width, 1), 117, dtype=np.uint8)
            img_path = batch_file_names[i]
            annot_path = img_path.replace("{}".format(self.img_type_prefix), "Annotations")[:-4]  # strip jpg
            ...
            # aux_channel = np.full((img_height, img_width, 1), 117, dtype=np.uint8)
            batch_x_auxillary.append(aux_channel)
            # print("shape of aux_channel : {}".format(aux_channel.shape))
    ...
    def dump_raw_data(self, batch_x_data, batch_y_data, batch_x_aux):
        # print("dumping data")
        temp_dir = "./temp"
    ...
    def generate(self,
        ...
        while True:
            ...
            #     batch_X, batch_X_aux = self.crop_images(batch_x_data=batch_X, batch_y_data=batch_X_aux)

            batch_X = self.apply_resolution_transform(batch_X)

            self.dump_raw_data(batch_x_data=batch_X.copy(), batch_y_data=batch_y, batch_x_aux=batch_X_aux)
