...
if __name__ == "__main__":
    ...
    ref_cam_frame_est = cv2.imread("{}/{}/{}.jpg".format(dir_name, view_1_name, "frame_0062"))
    ...
    view_1_name = "View_00{}".format(ref_cam)

    for c_cam in collab_cams:
        view_2_name = "View_00{}".format(c_cam)
        ...
        gt_box_coords_vw_1 = get_gt_sp_overlap_coordinates("0{}".format(ref_cam), "0{}".format(c_cam))
        gt_box_coords_vw_2 = get_gt_sp_overlap_coordinates("0{}".format(c_cam), "0{}".format(ref_cam))
        ...
        # frame_number = 1
        for frame_number in range(100, 550):
        ...
        cv2.putText(ref_cam_frame_est, "{}_{}".format(ref_cam, c_cam),
                    (est_box_coords_vw1_global[0] + 20, est_box_coords_vw1_global[1] - 5),
                    cv2.FONT_HERSHEY_COMPLEX_SMALL,
                    1.2, colors[color_index], 2)

        cv2.rectangle(ref_cam_frame_actual, (gt_box_coords_vw_1[0], gt_box_coords_vw_1[1]),
                      (gt_box_coords_vw_1[2], gt_box_coords_vw_1[3]), colors[color_index], 2)
        cv2.putText(ref_cam_frame_actual, "{}_{}".format(ref_cam, c_cam),
                    (gt_box_coords_vw_1[0] + 20, gt_box_coords_vw_1[1] - 5), cv2.FONT_HERSHEY_COMPLEX_SMALL,
                    1.2, colors[color_index], 2)
        color_index += 1

        cv2.imshow("ref_cam_frame_est", ref_cam_frame_est)
        cv2.imshow("ref_cam_frame_actual", ref_cam_frame_actual)
        cv2.imwrite("performance_analysis/spatial_area_estimation_PETS/cam_{}_{}_area.jpg".format(ref_cam, c_cam),
                    view_1_area)
        cv2.waitKey(-1)
        # break
