...
def match_objects(objects_1, objects_2):
    ...
    for obj1 in objects_1:
        for obj2 in objects_2:
            ...
            if euc_dist <= REID_EUCL_DIST:  # empirically chosen value
                obj_same = True
            else:
                obj_same = False
    ...
    for index, row in obj_mappings.iterrows():
        ...
        for r in range(y1, y2):
            for c in range(x1, x2):
                r = min(image_size[0] - 1, r)
                c = min(image_size[1] - 1, c)
        ...
        if DEBUG:
            view_1_area_resized = cv2.resize(view_1_area, (500, 500), cv2.INTER_AREA)
            view_2_area_resized = cv2.resize(view_2_area, (500, 500), cv2.INTER_AREA)
            cv2.imshow("camera_{}_area".format(ref_cam), view_1_area_resized)
            cv2.imshow("camera_{}_area".format(collab_cam), view_2_area_resized)
...
def get_all_objects_features(view_n_objects, view_n_frame):
    ...
    """
    view_n_obj_features = []
    for obj in view_n_objects:
        obj[0] = max(0, obj[0])
        obj[1] = max(0, obj[1])
        obj[2] = max(0, obj[2])
        obj[3] = max(0, obj[3])

        img = view_n_frame[obj[1]:obj[1] + obj[3], obj[0]:obj[0] + obj[2]]
        obj_location = [(obj[0], obj[1]), (obj[0] + obj[2], obj[1] + obj[3])]
        obj_track_id = 0
...
def draw_boxes(filename, obj_list):
    DATA_DIR = "../../../dataset/SMU_Setup/Spatial_Overlap/Sample_Images/viewer4"
    filename = "{}/{}.jpg".format(DATA_DIR, filename)

    image = cv2.imread(filename)
    assert image is not None
    for obj in obj_list:
        cv2.rectangle(image, (obj[0], obj[1]), (obj[0] + obj[2], obj[1] + obj[3]), (0, 255, 0), 2)
...
if __name__ == "__main__":
    ...
    for frame_number in range(10, 350, 8):  # 50% training data
        ...
        frame_name = "{}/{}/{}.jpg".format(dir_name, view_1_name, ref_frame_name)
        if not os.path.exists(frame_name):
            continue
        view_1_frame = cv2.imread("{}/{}/{}.jpg".format(dir_name, view_1_name, ref_frame_name))
        view_2_frame = cv2.imread("{}/{}/{}.jpg".format(dir_name, view_2_name, collab_frame_name))
        assert view_1_frame is not None
        assert view_2_frame is not None
        ...
        # view_1_objects = load_detected_objects("{}/{}/{}".format(dir_name, view_1_name, view_1_detected_objs))
        # view_2_objects = load_detected_objects("{}/{}/{}".format(dir_name, view_2_name, view_2_detected_objs))
        ...
        key_1 = "frame_{}_{}".format(ref_cam, frame_number)
        key_2 = "frame_{}_{}".format(collab_cam, frame_number)

        view_1_obj_features = get_all_objects_features(view_1_objects[key_1], view_1_frame)
        view_2_obj_features = get_all_objects_features(view_2_objects[key_2], view_2_frame)
        ...
        if DEBUG:
            ...
            cv2.putText(view_1_frame, "Actual Overlap", (gt_box_coords_vw_1[0], gt_box_coords_vw_1[1] + 20),
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.7, (0, 255, 0), 1)
            ...
            cv2.putText(view_1_frame, "Estimated Overlap",
                        ...
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.7, (0, 0, 255), 1)
            view_1_frame_resized = cv2.resize(view_1_frame, (500, 500), cv2.INTER_AREA)
            cv2.imshow("Camera_{}".format(ref_cam), view_1_frame_resized)

            draw_rect(est_box_coords_vw2_global, view_2_frame, (0, 0, 255))
            view_2_frame_resized = cv2.resize(view_2_frame, (500, 500), cv2.INTER_AREA)
            cv2.imshow("Camera_{}".format(collab_cam), view_2_frame_resized)
    ...
    plt.title("Camera {}".format(ref_cam), fontsize=18)
    ...
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    plt.title("Camera {}".format(collab_cam), fontsize=18)
