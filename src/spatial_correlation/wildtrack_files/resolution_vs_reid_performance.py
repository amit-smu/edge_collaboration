"""
performance of person re-identification with varying resolution of input images
"""
...
def extract_obj_features(view_num, view_img, frame_name, resolution):
    ...
    # rescale the image
    view_img = cv2.resize(view_img, dsize=resolution)
        ...
        for v in views:
            ...
            if v_num + 1 == view_num:  # starts from 0
                ...
                if x1 < 0 or x2 < 0 or y1 < 0 or y2 < 0:
                    ...
                    # rescale the image coordinates to new resolution
                    x1 = int((x1 / image_size[0]) * resolution[0])
                    y1 = int((y1 / image_size[1]) * resolution[1])
                    x2 = int((x2 / image_size[0]) * resolution[0])
                    y2 = int((y2 / image_size[1]) * resolution[1])

                    # extract embedding for these coordinates
                    person_img = view_img[y1:y2, x1:x2]
                    if DEBUG:
                        cv2.imshow("person_img_4", person_img)
                        cv2.waitKey(-1)
...
if __name__ == "__main__":
    ...
    resolutions = [(1920, 1080), (1280, 720), (1024, 576), (640, 480), (320, 240)]
    resolutions_xticks = ["1920x1080", "1280x720", "1024x576", "640x480", "320x240"]
    precisions = []
    recalls = []
    ...
    for res in resolutions:
        ...
        for frame_number in range(0, 2000, 5):
            ...
            view_1_obj_features = extract_obj_features(view_num=ref_cam, view_img=view_1_frame,
                                                       frame_name=frame_name, resolution=res)
            view_2_obj_features = extract_obj_features(view_num=collab_cams[0], view_img=view_2_frame,
                                                       frame_name=frame_name, resolution=res)

            est_equality, actual_equality = match_objects(view_1_obj_features, view_2_obj_features, 10)
        ...
        # print("resolution: {}, precision: {}, recall : {}".format(res, prec, recall))

    # plot the results
    print("resolutions: {}".format(resolutions))
    print("Precisions: {}".format(precisions))
    print("Recalls : {}".format(recalls))
    plt.plot(resolutions_xticks, precisions, marker="+", label="precision")
    plt.plot(resolutions_xticks, recalls, marker="*", label="recall")
    plt.legend()
